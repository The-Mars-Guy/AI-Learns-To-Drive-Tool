...
import {
  ...
  hiddenWrap, addHidden, nnXavierWeights, nnXavierBiases, nnPresetOutBias,
} from './dom.js';
...
let preview = { shape: null, weights: null, biases: null };
...
function ensurePreview(shape){
  const needWeights = !!nnXavierWeights?.checked;
  const needBiases  = !!nnXavierBiases?.checked || !!nnPresetOutBias?.checked;

  const toggSig = `${needWeights}/${needBiases}`;
  const prevSig = preview._sig;
  const shapeChanged = !preview.shape || preview.shape.length !== shape.length || preview.shape.some((v,i)=>v!==shape[i]);
  if (!shapeChanged && prevSig === toggSig) return;

  preview.shape = [...shape];
  preview._sig = toggSig;

  let base = buildZeroWeightsAndBiases(shape);

  if (needWeights) {
    const x = buildXavierWeightsAndBiases(shape);
    base.weights = x.weights;
  }

  if (needBiases) {
    const x = buildXavierWeightsAndBiases(shape);
    base.biases = x.biases;
  }

  if (nnPresetOutBias?.checked) {
    const out = shape[shape.length - 1];
    base.biases[base.biases.length - 1] = getPresetOutBiasVector(out);
  }
...
export function renderNNSvg(){
  ...
  let lines = '';
  if (nnXavierWeights?.checked && preview.weights) {
    const layerMaxAbs = [];
    for (let li = 0; li < L - 1; li++) {
      const w = preview.weights[li] || [];
      const rows = w.length;
      const cols = rows > 0 ? (w[0]?.length ?? 0) : (layers[li + 1] || 0);
      let m = 0;
      if (rows > 0 && cols > 0) {
        for (let r = 0; r < rows; r++) {
          for (let c = 0; c < cols; c++) {
            const v = Math.abs(w[r]?.[c] ?? 0);
            if (v > m) m = v;
          }
        }
      }
    ...
    const weightStroke = (li, ri, ci) => {
      const W = preview.weights?.[li];
      if (!W || !W.length || !W[ri] || typeof W[ri][ci] !== 'number') {
        return 'rgba(255,255,255,0.15)';
      }
      const v = W[ri][ci];
      const sign = v >= 0 ? 1 : -1;
      const norm = Math.min(1, Math.abs(v) / (layerMaxAbs[li] || 1e-6));
    ...
    for (let li = 0; li < L - 1; li++) {
      ...
      for (let r = 0; r < y1s.length; r++) {
        for (let c = 0; c < y2s.length; c++) {
          lines += `<line x1="${xs[li]}" y1="${y1s[r]}" x2="${xs[li + 1]}" y2="${y2s[c]}" stroke="rgba(255,255,255,0.15)" stroke-width="1"/>`;
        }
      }
    }
  }
  ...
  for (let li = 0; li < L; li++) {
    ...
    } else if (nnXavierBiases?.checked && preview.biases) {
      const bVec = preview.biases[li - 1];
      nodeFills = Array.from({ length: layers[li] }, (_, j) => {
        const b = bVec?.[j] ?? 0;
        return b >= 0 ? '#22c55e' : '#ef4444';
      });
    } else {
...
export function wireNNToggles(onChange){
  ...
  nnXavierWeights?.addEventListener('change', onChange);
  nnXavierBiases?.addEventListener('change', onChange);
